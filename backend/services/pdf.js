...
async function generateAndUploadPDF(reportId) {
  ...
  try {
    ...
    const tableRows = sortedValues.map(tv => {
      ...
      return `
        <tr class="${tv.flag !== 'normal' ? 'abnormal-row' : ''}">
          <td>${p?.name || '—'}</td>
          <td class="val-cell">${tv.value}</td>
          <td>${p?.unit || '—'}</td>
          <td>${refStr}</td>
          <td><span class="flag-badge ${cls}">${label}</span></td>
        </tr>`;
    }).join('');
    ...
    const html = templateRaw
      ...
      .replace(/{{COLLECTED_DATE}}/g, report.collected_at
        ? new Date(report.collected_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })
        : 'N/A')
      .replace(/{{REPORTED_DATE}}/g, report.reported_at
        ? new Date(report.reported_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })
        : 'N/A')
