...
const REPORT_JSON_SCHEMA = `{
    ...
    "parameters": [
      {
        ...
        "high": number | null,
        "status": "Normal" | "Abnormal" | "Borderline",
        "insight": "Plain language explanation of what this marker means",
        "creativeSolution": "One actionable food/lifestyle improvement for this result",
        "suggestedMedicine": "General medicine category or 'None' if normal",
        "suggestedSpecialist": "Specific type of doctor to consult"
      }
    ],
  ...
  "summary": "Detailed 3-4 sentence plain-language clinical summary with key findings and health advice",
  "advice": string[],
  "riskLevel": "Low" | "Medium" | "High"
}`;
...
function normalizeOcrResult(parsed = {}) {
  return {
    ...
    summary: parsed.summary || 'Medical report successfully analyzed.',
    advice: Array.isArray(parsed.advice) && parsed.advice.length ? parsed.advice : ['Consult your doctor for clinical interpretation of these results.'],
    riskLevel: parsed.riskLevel || 'Medium',
  };
}
...
async function analyzeReportImage(base64Image, mimeType = 'image/jpeg') {
  ...
  if (!client) {
    logger.warn('OCR: GROQ_API_KEY not configured — returning fallback');
    return normalizeOcrResult({ summary: 'AI is not configured. Please contact support.' });
  }

  // ─── PATH A: PDF — extract text and analyze with a text model ───────────
  if (mimeType === 'application/pdf') {
    logger.info('OCR: PDF detected — extracting text with pdf-parse');
    let pdfText = '';
    try {
    ...
    if (!pdfText || pdfText.trim().length < 30) {
      logger.warn('OCR: PDF text too short — falling through to image path');
      // Let it fall to image analysis below
    } else {
      try {
        const prompt = `You are an expert medical document analyzer.
Extract EVERY visible test result, biomarker, medicine, patient detail and lab detail from this text.
...
${REPORT_JSON_SCHEMA}`;
        ...
        const parsed = extractJsonObject(pdfResponse.choices[0]?.message?.content || '{}');
        logger.info('OCR: PDF text analysis complete', { params: parsed.results?.parameters?.length || 0 });
        return normalizeOcrResult(parsed);
      } catch (textErr) {
        logger.error(`OCR PDF text analysis failed: ${textErr.message} — falling through to image path`);
      }
    }
  }

  // ─── PATH B: Image (or PDF fallback via rendered canvas) — vision model ─
  try {
          ...
          messages: [
            {
              ...
              content: [
                {
                  type: 'text',
                  text: `You are an expert medical report analyzer. Extract EVERY test result, biomarker, and all patient/lab details from this medical document image.
...
${REPORT_JSON_SCHEMA}`,
        ...
        logger.info(`OCR: Vision model ${model} succeeded`);
        break;
      } catch (err) {
        lastError = err;
        logger.warn(`OCR: ${model} failed — ${err.message}`);
      }
    }
    ...
    logger.info('OCR: Vision response received', { length: content.length });
    const parsed = extractJsonObject(content);
    return normalizeOcrResult(parsed);

  } catch (err) {
    logger.error(`OCR vision analysis failed: ${err.message}`, { status: err.status });
    // Return a descriptive error payload (not the generic "OCR failed" message)
    return normalizeOcrResult({
      summary: `We could not automatically extract data from this document. This can happen with handwritten reports or very low-resolution scans. Please ensure the document is a typed/printed PDF or a clear photograph. Error: ${err.message}`,
      advice: [
        'Upload a clear, well-lit photograph of the report.',
        'Use a text-based (typed) PDF rather than a scanned image PDF.',
        'Ensure all text is sharp and readable before uploading.',
      ],
      riskLevel: 'Unknown',
    });
  }
