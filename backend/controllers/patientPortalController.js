...
async function getMyReport(req, res) {
  ...
  try {
    ...
    logger.error(`Detailed AI analysis failed for report ${report_id}: ${msg}`);
  }
...
async function sendLinkPhoneOtp(req, res) {
  ...
  if (error) {
    logger.error(`Failed to create link OTP session for ${phone}: ${error?.message || error}`);
...
async function verifyLinkPhoneOtp(req, res) {
  ...
  if (targetPatient) {
    ...
    if (updateErr) {
      logger.error(`Failed to link phone number for ${current_patient_id}: ${updateErr?.message || updateErr}`);
