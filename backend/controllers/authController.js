...
async function sendOtpHandler(req, res) {
  ...
  // Use real SMS if Twilio config is present, otherwise fallback to Dev Mode (fixed OTP 123456)
  ...
  if (isDev) {
    ...
    try {
      ...
      // Fallback: update the session to use 123456 so they can still test
      const fallbackOtp = '123456';
      const hashedFallback = await bcrypt.hash(fallbackOtp, 10);
      await supabase.from('otp_sessions').update({ hashed_otp: hashedFallback }).eq('phone', phone);

      return res.status(200).json({
        message: `SMS delivery failed (${err.message}). Falling back to Dev Mode: Use 123456`,
        isFallback: true
      });
    }
  }
...
async function verifyOtpHandler(req, res) {
  ...
  const isDev = !process.env.TWILIO_ACCOUNT_SID; // Match sendOtpHandler
