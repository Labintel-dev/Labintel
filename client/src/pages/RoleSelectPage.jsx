...
const RoleSelectPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'register') {
      const existing = findUserByRoleAndEmail(selectedRole.id, email);
      if (existing) {
        setError('Email is already registered. Please sign in instead.');
      } else {
        const createdUser = registerUser(name, email, phone, dob, password);
        setUser(createdUser);
    ...
    const user = authenticate(authId, email, password);
    if (user) {
      setUser(user);
      navigate(selectedRole.path);
    } else {
