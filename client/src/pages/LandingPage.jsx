...
const LandingProfileUpdateModal = ({ user, onClose, onUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (file && authId) {
        avatarUrl = await uploadAvatar(authId, file);
      }
          ...
          <div className="flex flex-col items-center mb-6">
            <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
              {preview ? (
                ...
                <div className="w-24 h-24 rounded-full flex items-center justify-center text-white text-3xl font-bold border-4 border-white shadow-md" style={{ background: P }}>
                  {String(name || user?.name || 'U').charAt(0).toUpperCase()}
                </div>
              )}
...
<<<<<<< HEAD
      ...
      <AnimatePresence>
        {isUpdateModalOpen && currentUser && (
          <LandingProfileUpdateModal
            user={currentUser}
            onClose={() => setIsUpdateModalOpen(false)}
            onUpdated={(updatedUser) => {
              setProfileDraft((prev) => ({ ...(prev || {}), ...updatedUser }));
              try {
                const raw = localStorage.getItem('labintel_user');
                if (raw) {
                  const parsed = JSON.parse(raw);
                  const next = { ...parsed, ...updatedUser };
                  localStorage.setItem('labintel_user', JSON.stringify(next));
                }
              } catch {
                // no-op local cache update
              }
            }}
          />
        )}
