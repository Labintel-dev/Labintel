...
import {
  ...
  TEST_OPTIONS, CATEGORY_OPTIONS, subscribeToReports
} from '../lib/supabase';
...
const toSafeFilename = (value = 'diagnostic-report') =>
  String(value)
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '') || 'diagnostic-report';

const getDownloadFilename = (report) => {
  const baseName = report?.fileName || `${report?.testName || 'diagnostic-report'}.pdf`;
  if (/\.[a-z0-9]+$/i.test(baseName)) return baseName;
  return `${toSafeFilename(baseName)}.pdf`;
};
...
const fileToDataUrl = (file) =>
  new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error('Failed to read the selected report file.'));
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 24 }}
          ...
          <div className="p-5 border-t border-gray-100 flex items-center justify-between">
            ...
            <TealBtn onClick={() => (downloadReportFile(report) || window.print())} size="sm">
              <Download size={12} /> {report.downloadUrl ? 'Download Report' : 'Download PDF'}
            </TealBtn>
          </div>
        </motion.div>
      </motion.div>
...
const ReportCard = ({ report, onAction, index }) => (
  <motion.div
  ...
  >
    ...
    <Badge status={report.status} />
    <TealBtn size="sm" onClick={() => onAction(report)} disabled={report.status !== 'Ready'}>
      <Download size={12} /> {report.downloadUrl ? 'Download' : 'View'}
    </TealBtn>
  </motion.div>
);
...
const PatientReports = ({ user }) => {
  ...
  useEffect(() => {
    ...
    const channel = subscribeToReports(user.id, () => {
      fetchReports();
    });
  ...
  const downloadableReports = reports.filter(r => r.status === 'Ready' && r.downloadUrl);
    ...
    <>
      ...
      <div className="bg-white rounded-3xl border border-gray-200 p-5 mb-4">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between mb-4">
          <div>
            <h2 className="text-base font-bold text-gray-800">Download My Report</h2>
            <p className="text-xs text-gray-400">
              Reports uploaded by the lab technician will appear here automatically.
            </p>
          </div>
          <div className="text-xs font-semibold text-[#14453d] bg-[#e8f5f0] px-3 py-1.5 rounded-xl">
            {downloadableReports.length} ready to download
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2.5">
            {downloadableReports.map((report, index) => (
              <ReportCard key={`download-${report.id}`} report={report} onAction={handleReportAction} index={index} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-14 text-center bg-[#f8faf9] rounded-2xl border border-dashed border-gray-200">
            <Download size={30} className="text-gray-300 mb-3" />
            <p className="text-sm font-semibold text-gray-600">No downloadable reports yet</p>
            <p className="text-xs text-gray-400 mt-1 max-w-sm">
              Once your technician uploads a completed report, you will be able to download it from this section.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-2.5">
        {loading ? (
          <div className="flex items-center justify-center py-14 text-sm text-gray-400 bg-white rounded-2xl border border-gray-200">
            Loading your reports...
          </div>
        ) : visible.length > 0
          ? visible.map((r, i) => (
              <ReportCard key={r.id} report={r} onAction={handleReportAction} index={i} />
            ))
          : (
            ...
            <motion.div key={r.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex-1 mb-4">
                <div className="bg-white rounded-2xl border border-gray-200 p-4
                  ...
                  <TealBtn size="sm" onClick={() => (r.downloadUrl ? downloadReportFile(r) : setActiveReport(r))}
                           disabled={r.status !== 'Ready'}>
                    <Download size={12} /> {r.downloadUrl ? 'Download' : 'View'}
                  </TealBtn>
                </div>
              </div>
            </motion.div>
...
const LabUpload = ({ user }) => {
  ...
  const handleSubmit = async () => {
    if (!testName || !patientId || !file) return;
    setLoading(true);
    try {
            ...
            <AnimatePresence>
              {success && (
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <CheckCircle size={14} />
                  <span>Report uploaded and made available for patient download.</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>

          <TealBtn onClick={handleSubmit} disabled={!testName || !file} className="w-full justify-center mt-4">
