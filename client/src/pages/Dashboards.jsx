...
import { getServerHealth } from '../lib/serverApi';
...
const Shell = ({ navItems, showSearch = false, renderContent }) => {
  ...
  useEffect(() => {
    async function checkUser() {
      ...
      // Map Supabase user to the UI user state
      setUserState({
        id: authUser.id,
        name: authUser.user_metadata?.full_name || authUser.email.split('@')[0],
        email: authUser.email,
        role: authUser.user_metadata?.role || 'patient',
        avatar: (authUser.user_metadata?.full_name || authUser.email).charAt(0).toUpperCase()
      });
    }
    checkUser();
  }, [navigate]);
