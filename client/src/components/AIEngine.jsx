...
<<<<<<< HEAD
      ...
      if (isAiCapacityError(err)) {
        ...
        onAnalysisComplete(buildAiCapacityFallbackPayload({ retryAfterSec }));
        return;
      }
      ...
      const retryAfterSec = Number(err.response?.data?.retryAfterSec || 0);
      let msg = 'Analysis failed. Please try a clearer image.';
      if (status === 429 || apiCode === 'AI_QUOTA_EXCEEDED') {
        msg = retryAfterSec > 0
          ? `AI quota is temporarily exhausted. Please retry in about ${retryAfterSec} seconds.`
          : 'AI quota is temporarily exhausted. Please retry shortly.';
      } else if (status === 503 || apiCode === 'AI_BUSY') {
        msg = 'AI is under heavy load right now. Please retry in a few seconds.';
