...
- [Features](#-features)
- [PDF Report Changes by Soumya](#-pdf-report-changes-by-soumya)
...
- **AI-Powered Insights** — Google Gemini integration for intelligent report summaries
...
## PDF Report Changes by Soumya

- Reworked the PDF report UI in `backend/templates/report.html` into a cleaner branded diagnostic report layout.
- Added a stronger report header with lab identity, contact details, patient details, registration and report timestamps, and a centered test panel title.
- Improved report identification visuals in generated PDFs, including barcode- and QR-style blocks.
- Refined the results table presentation with grouped investigations, clearer units and reference ranges, and better highlighting for abnormal or borderline values.
- Added patient-friendly summary, interpretation, and instrument details sections to the PDF output.
- Upgraded `backend/services/pdf.js` to prepare richer template data with safer HTML escaping, formatted dates, and dynamic placeholder replacement.
- Added logic to include registration-related details such as patient code, referring doctor, and registration timestamp in generated reports.
- Added duplicate test-value filtering before rendering the PDF so repeated rows do not appear in the final output.
- Added fallback branding assets for PDF generation, including inline SVG rendering when a logo file is unavailable.
- Updated Puppeteer PDF generation settings for a full-page A4 layout with print-focused styling.
