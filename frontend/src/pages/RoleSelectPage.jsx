...
import {
  User,
  Mail, Lock, Eye, EyeOff, AlertCircle, CheckCircle, Smartphone, Calendar
} from 'lucide-react';
import { Navbar } from './LandingPage';
import { signIn, signUp } from '../lib/supabase';
import { upsertServerProfile } from '../lib/serverApi';
// import { authenticate, findUserByRoleAndEmail, setUser, registerUser } from '../data/mockData';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'register') {
        ...
        console.log('Supabase signUp response:', { user: !!user, session: !!session });

        // If your Supabase is set to "Confirm Email", session will be null here.
        const token = session?.access_token;
        if (token) {
          console.log('Session found, syncing profile with server...');
          await upsertServerProfile(token, {
            full_name: name,
            phone,
            dob,
            role: DEFAULT_PORTAL.id
          });
          setSuccess('Registration successful! Redirecting...');
          setTimeout(() => navigate('/?login=success'), 1500);
        } else {
          console.warn('No session returned. User probably needs to confirm email first.');
          setSuccess('Registration initiated! Please check your email inbox to confirm your account first. After confirming, you can sign in.');
        }
      ...
      if (mode === 'forgot') {
        // Simple placeholder for forgot password
      ...
      navigate('/patient?login=success');
    } catch (err) {
             ...
             >
                <div className="mb-8 text-center flex flex-col items-center">
                  ...
                  <p className="text-sm text-gray-500 max-w-[280px]">
                    {mode === 'register' ? 'Create your patient account to get started.' : mode === 'forgot' ? 'Enter your email to receive a secure password reset link.' : DEFAULT_PORTAL.desc}
                  </p>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                    <p className="text-sm text-gray-500">
                      New here? <button onClick={() => setMode('register')} className="font-bold text-[#14453d] hover:underline transition-all">Create an account</button>
                    </p>
                  {mode !== 'login' && (
