...
export default function Patients() {
  ...
  return (
    <LabLayout>
                  ...
                  {patients.map((lp) => {
                    ...
                    return (
                      <tr key={lp.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 text-slate-500 hidden md:table-cell text-xs">{formatDate(lp.registered_at)}</td>
      ...
      <Modal open={showCreate} onClose={() => { setShowCreate(false); reset(); }} title="Register New Patient" className="max-w-lg">
        <form onSubmit={handleSubmit((d) => createMutation.mutate(d))} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <Input label="Full Name *" {...register('full_name')} error={errors.full_name?.message} placeholder="Patient's full name" />
            </div>
            <div className="col-span-2">
            ...
            <Input label="Internal Code" {...register('lab_patient_code')} placeholder="e.g. SUN-042" />
            <Input label="Referred By" {...register('referred_by')} placeholder="Dr. Name" />
          </div>
          <div className="flex gap-2 pt-2">
            <Button type="submit" className="flex-1" isLoading={createMutation.isPending}>Register Patient</Button>
