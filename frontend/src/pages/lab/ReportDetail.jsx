...
export default function ReportDetail() {
  ...
  const insight = Array.isArray(report?.report_insights) ? report.report_insights[0] : report?.report_insights;
      ...
      <div className="max-w-4xl mx-auto space-y-5">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {(report.test_values || []).map((tv, i) => {
                  ...
                  return (
                    <tr key={i} className={tv.flag && tv.flag !== 'normal' ? 'bg-red-50/50' : ''}>
                      <td className="px-5 py-3 font-medium text-slate-800">
                        {typeof p?.name === 'object' ? JSON.stringify(p.name) : (p?.name || 'Unknown')}
                      </td>
                      <td className="px-3 py-3 text-center font-bold text-slate-800">
                        {typeof tv.value === 'object' ? JSON.stringify(tv.value) : (tv.value || '--')}
                      </td>
                      <td className="px-3 py-3 text-center text-slate-500">
                        {typeof p?.unit === 'object' ? JSON.stringify(p.unit) : (p?.unit || '—')}
                      </td>
                      <td className="px-3 py-3 text-center text-slate-500">
        ...
        <div className="rounded-xl border-2 border-teal-200 bg-teal-50 p-5">
          ...
          {!insight ? (
            ...
            <>
              <p className="text-sm text-teal-900 mb-3 whitespace-pre-wrap">
              ...
              {Array.isArray(insight.findings) && insight.findings.length > 0 && (
              ...
              {insight.recommendation && (
                <div className="bg-white/60 rounded-lg p-3 text-sm italic text-teal-800 border border-teal-200">
                  💡 {typeof insight.recommendation === 'string' ? insight.recommendation : JSON.stringify(insight.recommendation, null, 2)}
                </div>
              )}
            </>
          )}
