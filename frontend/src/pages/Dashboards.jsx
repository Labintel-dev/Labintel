...
const ReportCard = ({
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-3xl border border-gray-100 p-4 sm:p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-xl hover:border-teal-100 transition-all group overflow-hidden relative"
    >
      {/* Info row */}
      <div className="flex items-start justify-between gap-4 relative z-10">
        <div className="flex items-center gap-3 sm:gap-5 min-w-0">
          <div className="w-11 h-11 sm:w-14 sm:h-14 bg-[#f0f9f6] rounded-[1.25rem] flex items-center justify-center shrink-0 group-hover:bg-[#e8f5f0] transition-colors shadow-sm">
            <FileText size={20} className="text-[#14453d]" />
          </div>
          <div className="min-w-0">
            <div className="text-base sm:text-lg font-black text-gray-800 tracking-tight leading-tight mb-1 truncate">{report.testName}</div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[10px] sm:text-[11px] font-black text-[#14453d]/60 uppercase tracking-widest">{report.labName}</span>
              <div className="w-1 h-1 rounded-full bg-gray-300 hidden sm:block" />
              <span className="text-[10px] sm:text-[11px] font-bold text-gray-400 uppercase tracking-tighter">{report.date}</span>
            </div>
          </div>
        </div>
      ...
      {isReady ? (
        <div className="mt-5 pt-5 border-t border-gray-100 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button
            ...
            className="flex items-center justify-center gap-2 py-3.5 bg-gradient-to-r from-[#14453d] to-[#1a5a50] text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] hover:opacity-90 hover:-translate-y-0.5 transition-all shadow-lg shadow-[#14453d]/20 active:scale-95 group w-full"
          >
            <Sparkles size={14} className="text-emerald-400 group-hover:animate-pulse shrink-0" />
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 py-3.5 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] transition-all active:scale-95 border w-full ${
              isVoicePlaying
                ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
                : 'bg-white text-emerald-600 border-emerald-100 hover:bg-emerald-50 hover:text-emerald-700'
            } ${isVoiceLoading ? 'opacity-70 cursor-wait' : ''}`}
          >
            <Volume2 size={14} className={`shrink-0 ${isVoicePlaying ? 'animate-pulse' : ''}`} />
            <span>{isVoiceLoading ? 'Loading...' : isVoicePlaying ? 'Stop Voice' : 'Voice Summary'}</span>
          </button>
          <button
            type="button"
            onClick={() => onSourcePdf(report)}
            disabled={isSourcePdfLoading}
            className={`flex items-center justify-center gap-2 py-3.5 bg-white border border-gray-200 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] transition-all active:scale-95 w-full ${
              isSourcePdfLoading ? 'text-gray-500 opacity-80 cursor-wait' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-600'
            }`}
          >
            <Download size={14} className="shrink-0" />
            <span>{isSourcePdfLoading ? 'Opening...' : 'Source PDF'}</span>
          </button>
        </div>
      ) : (
...
const PatientReports = ({ user, onAnalyze }) => {
  ...
  const buildVoiceSummaryText = (report, detailedReport = null) => {
    ...
    const summary =
      detailedReport?.ai_analysis?.hindiVoiceScript ||
      detailedReport?.ai_analysis?.summary ||
      reportInsights?.summary ||
      report?.ai_analysis?.hindiVoiceScript ||
  ...
  const handleVoiceSummary = async (report) => {
    ...
    try {
      ...
      // Helper: load voices asynchronously (Chrome/Edge load voices lazily)
      ...
      // Step 1: Call Backend to get Hindi Translation & Audio Chunks
      let hindiText = null;
      let audioData = null;
      try {
        const ttsResponse = await apiClient.post('/ai/voice-summary-public', { text: summaryText });
        hindiText = ttsResponse.data?.hindiText;
        audioData = ttsResponse.data?.audioData;
      } catch (ttsErr) {
        console.warn('Backend translation/TTS failed:', ttsErr);
      }

      // Step 2: Play MP3 audio chunks from backend (highly reliable, works everywhere)
        ...
        const playNext = () => {
          if (currentIndex < audios.length) {
            audios[currentIndex].play().catch((e) => {
               console.warn("Backend audio playback error:", e);
               setVoicePlayingReportId(null);
            });
            audios[currentIndex].onended = () => { currentIndex++; playNext(); };
          } else {
            setVoicePlayingReportId(null);
            setAudioElements([]);
          }
      ...
      // Step 3: Try Browser Speech Synthesis with the translated Hindi text (fallback)
      if (hindiText && typeof window !== 'undefined' && window.speechSynthesis) {
        try {
          ...
          if (hindiVoice) {
            ...
            utterance.onerror = (e) => { 
               console.warn('Browser speech failed during playback:', e);
               setVoicePlayingReportId(null); 
            };
          ...
          console.warn('Browser speech synthesis setup failed:', synthErr);
        }
      }

      // Step 4: Absolute fallback — use browser speech in English if all else fails
      if (typeof window !== 'undefined' && window.speechSynthesis) {
        ...
        addToast("Voice playback is not supported on this device.", "error");
      }

    } catch (err) {
      console.error('Voice summary failed:', err);
      addToast("Failed to generate voice summary. Please try again.", "error");
    } finally {
      setVoiceLoadingReportId(null);
    }
  };
    ...
    <div className="relative">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6">
        {[
          ...
          <div key={s.label}
            className={`${s.bg} rounded-2xl border border-gray-200 p-3 sm:p-4 text-center shadow-sm`}>
            <div className={`text-2xl sm:text-3xl font-bold ${s.color}`}>{s.value}</div>
            <div className="text-[10px] sm:text-xs uppercase font-bold text-gray-400 tracking-wide mt-1">
              {s.label}
            </div>
          </div>
        ))}
      ...
      {/* Filter tabs */}
      <div className="flex gap-1 mb-4 bg-white rounded-xl p-1 border border-gray-200 overflow-x-auto max-w-full inline-flex sm:flex">
        {tabs.map(t => (
          <button key={t} onClick={() => setFilter(t)}
            className={`text-[11px] sm:text-xs font-bold px-3 sm:px-4 py-2 rounded-lg transition-all whitespace-nowrap
                    ${filter === t ? 'bg-[#14453d] text-white shadow-sm' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-50'}`}>
            {t}
          </button>
        ))}
      </div>
