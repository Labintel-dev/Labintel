...
const Shell = ({ navItems, showSearch = false, renderContent }) => {
  ...
  const fetchFullProfile = async () => {
    try {
      ...
      // Fallback: look up patient by phone from user_metadata
      if (!profile) {
        const metaPhone = authUser.user_metadata?.phone;
        if (metaPhone) {
          const normDigits = metaPhone.replace(/\D/g, '');
          const searchPhone = normDigits.length === 10 ? '+91' + normDigits : '+' + normDigits;
      ...
      setUserState({
        ...
        avatar: (profile?.full_name || authUser.email || 'U').charAt(0).toUpperCase()
      });
    } catch (err) {
