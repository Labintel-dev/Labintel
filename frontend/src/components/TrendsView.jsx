  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Parse results for trends
        const readyReports = data
          .filter(r => r.status === 'released')
          .map(r => ({
            date: r.reported_at || r.collected_at || r.created_at,
            results: (r.test_values || []).map(tv => ({
        ...
        // Set default marker if not in list
        const markersList = Array.from(markers);
        if (markersList.length > 0 && !markersList.includes(selectedMarker)) {
          setSelectedMarker(markersList[0]);
        }
