...
const OCRScanningWorkspace = ({ user }) => {
  ...
  const startProcessing = async (selectedFile) => {
    ...
    try {
      ...
      if (isPdfFile) {
        setSubStep(0); // Reading PDF...
        // Send the raw PDF base64 — backend extracts text with pdf-parse (much better quality)
        const reader = new FileReader();
        ...
        // For images: read → compress → send as JPEG
        const reader = new FileReader();
        base64 = await new Promise((resolve) => {
          reader.onload = (e) => resolve(e.target.result);
          reader.readAsDataURL(selectedFile);
      ...
      // SINGLE STEP MULTIMODAL ANALYSIS
      setSubStep(0);
