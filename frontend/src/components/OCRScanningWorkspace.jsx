...
const OCRScanningWorkspace = ({ user }) => {
  ...
  // Compress image to reduce payload size and API processing time
  const compressImage = async (base64String) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Downscale if image is too large (max 2400px width)
        if (width > 2400) {
        ...
        // Use quality 0.85 for good balance between quality and size
  ...
  const startProcessing = async (selectedFile) => {
    ...
    try {
      ...
      if (err.code === 'ECONNABORTED' || err.message?.includes('timeout')) {
        errMsg = 'Request timed out. The AI server is taking longer than usual. Please try again in a moment.';
      } else if (err.response?.status === 401) {
        errMsg = 'Session expired. Please refresh the page and log in again.';
      } else if (err.response?.status === 403) {
        errMsg = 'Access denied. Your account may not be set up as a staff member. Please contact your administrator.';
      } else if (err.response?.status === 429) {
        errMsg = 'Too many requests. Please wait a few minutes before trying again.';
      } else if (err.response?.status === 503) {
        errMsg = 'The AI server is currently overloaded. Please try again in 30 seconds.';
