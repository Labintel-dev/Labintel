...
// ─── Patient: Get fresh signed PDF URL ───────────────────────────────────
async function downloadMyReport(req, res) {
  ...
  const labintelBase = process.env.LABINTEL_API_BASE_URL;
  if (labintelBase && report.share_token) {
    try {
      const shareUrl = `${labintelBase.replace(/\/$/, '')}/api/v1/reports/share/${report.share_token}`;
      const { data } = await axios.get(shareUrl, { timeout: 8000 });
      const pdfUrl = data?.data?.pdf_url;
      if (pdfUrl) return res.json({ url: pdfUrl });
    } catch (err) {
      logger.warn(`Labintel proxy download failed for report ${report_id}: ${err.message}`);
    }
  }
