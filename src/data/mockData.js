...
// ── Users ──────────────────────────────────────────────────────────────────
export const USERS = [
  { id: 'P001', name: 'Rahul Mehta',   email: 'rahul@email.com',      role: 'patient', avatar: 'R' },
  { id: 'P002', name: 'Divya Reddy',   email: 'divya@email.com',      role: 'patient', avatar: 'D' },
  { id: 'S001', name: 'Priya Sharma',  email: 'priya@labintel.in',    role: 'staff',   avatar: 'P' },
  { id: 'S002', name: 'Arjun Mehta',   email: 'arjun@labintel.in',    role: 'staff',   avatar: 'A' },
  { id: 'D001', name: 'Dr. John Doe',  email: 'doctor@labintel.in',   role: 'doctor',  avatar: 'D' },
  { id: 'A001', name: 'Soumya Kumar',  email: 'admin@labintel.in',    role: 'admin',   avatar: 'S' },
];
...
export const getUser    = ()  => JSON.parse(localStorage.getItem('labintel_user')    || 'null');
export const setUser    = (u) => localStorage.setItem('labintel_user', JSON.stringify(u));
export const clearUser  = ()  => localStorage.removeItem('labintel_user');

export const getReports = () => {
  const stored = localStorage.getItem('labintel_reports');
  if (stored) return JSON.parse(stored);
  localStorage.setItem('labintel_reports', JSON.stringify(INITIAL_REPORTS));
  return INITIAL_REPORTS;
};
export const saveReports = (r) => localStorage.setItem('labintel_reports', JSON.stringify(r));

// ── Auth — checks email only (password validation skipped for now) ─────────
export const authenticate = (role, email) => {
  return USERS.find(u => u.role === role && u.email.toLowerCase() === email.toLowerCase()) || null;
};

export const registerUser = (name, email, phone, dob) => {
  const newUser = {
