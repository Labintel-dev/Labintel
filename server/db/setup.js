// Create tables and seed data using Supabase Management API
...
async function run() {
  ...
  try {
    ...
    console.log(`pg/query response (${res.status}):`, text.substring(0, 200));
    if (res.ok) { created = true; console.log('✅ Tables created via pg/query!'); }
  } catch (e) { console.log('pg/query failed:', e.message); }

  if (!created) {
    console.log('\n⚠️  Cannot create tables programmatically.');
    console.log('📋 Please create them manually:\n');
    console.log('1. Go to: https://supabase.com/dashboard/project/jjwhyykzifckmkfdakfx/sql/new');
    console.log('2. Log in to your Supabase account');
    console.log('3. Paste the SQL from server/db/schema.sql');
    console.log('4. Click "Run"');
    console.log('5. Then run: node db/seed.js\n');
    return;
  }
  ...
  if (labErr) { console.error('❌ Lab error:', labErr.message); return; }
  ...
  if (teamErr) { console.error('❌ Team error:', teamErr.message); return; }
  ...
  if (actErr) { console.error('❌ Activity error:', actErr.message); return; }
  console.log(`✅ Seeded ${activity.length} activity logs`);

  // Seed sessions
  const { data: sessions, error: sesErr } = await supabase.from('sessions').insert([
    { lab_id: labs.find(l => l.code === 'AP').id, lab: 'Apollo Diagnostics', subdomain: 'apollolab.labintel.in', active_count: 34, color: '#00d4aa' },
    { lab_id: labs.find(l => l.code === 'SR').id, lab: 'Sunrise Diagnostics', subdomain: 'sunrise.labintel.in', active_count: 22, color: '#06b6d4' },
    { lab_id: labs.find(l => l.code === 'CL').id, lab: 'City Lab Mumbai', subdomain: 'citylab.labintel.in', active_count: 16, color: '#3b82f6' },
    { lab_id: labs.find(l => l.code === 'MP').id, lab: 'MedCare Pathology', subdomain: 'medcare.labintel.in', active_count: 18, color: '#6366f1' },
    { lab_id: labs.find(l => l.code === 'HF').id, lab: 'HealthFirst Labs', subdomain: 'healthfirst.labintel.in', active_count: 7, color: '#8b5cf6' },
  ]).select();
  if (sesErr) { console.error('❌ Session error:', sesErr.message); return; }
